...
The complete list of settings is [`agenda.conf.example`](../agenda.conf.example), which is
in systemd `EnvironmentFile` format so the same templated file works as `EnvironmentFile=` in
a unit or as `agenda --config <path>`.

## What the application provides

| Interface | Contract |
|---|---|
| `agenda serve` | Long-running process. Listens on `AGENDA_LISTEN`. Exits non-zero on a configuration or migration failure, with the problems listed on stderr. |
| `agenda bootstrap --email <address> --name <name>` | Creates the first account and calendar on an empty database and prints an invite link. Signup is invite-only and there is no HTTP route to the first account, deliberately: the bootstrap window is never reachable from the internet. Refuses to run once any account exists. |
| `agenda backup <dir> [--prune]` | Takes a verified snapshot (`VACUUM INTO` → `PRAGMA integrity_check` on the output → fsync → atomic rename). **Exits non-zero if the snapshot is not intact**, which is the signal to alert on. `--prune` applies the generational retention from the config. |
| `agenda gen-vapid` | Prints a fresh VAPID keypair. Run once, ever, at first deployment. |
| `agenda seed` | Creates a demo family. Development only. |
| `agenda version` | Prints the build version. |
...
`AGENDA_TRUSTED_PROXIES`); do not add a `Content-Security-Policy` header, as the application
...
**A local MTA.** Listening on `AGENDA_SMTP` (`127.0.0.1:25` by default), configured to relay to
...
**Backups on a timer.** Run `agenda backup <dir> --prune` hourly, as the service user. Sync the
directory off-host at least daily, ordered after the snapshot, copying only `agenda-*.db` and
...
1. Generate the VAPID keypair (`agenda gen-vapid`) and put it in the secret store. Never rotate it.
2. Template the config file from `agenda.conf.example`.
...
7. Run `agenda bootstrap --email you@example.org --name "Maman"` on the host. Sign in with the
