...
func runSeed(ctx context.Context, cfg config.Config, force bool) error {
	...
	// --- people -------------------------------------------------------------
	people := []seedUser{
		{name: "Maman", email: "maman@example.org", color: "#c0392b", lang: domain.LangFR, admin: true},
		{name: "Papa", email: "papa@example.org", color: "#2980b9", lang: domain.LangFR},
		{name: "Léo", email: "leo@example.org", color: "#27ae60", lang: domain.LangFR},
		{name: "Mamie", email: "mamie@example.org", color: "#8e44ad", lang: domain.LangEN},
	}
	...
	for _, c := range []calSpec{
		{"Famille", "#3b7ddd", "Maman", []string{"Papa", "Léo", "Mamie"}},
		{"Parents", "#7d3bdd", "Maman", []string{"Papa"}},
		{"Activités enfants", "#2fa84f", "Papa", []string{"Maman", "Léo"}},
	} {
	...
	// --- events -------------------------------------------------------------
	famille, parents, enfants := "Famille", "Parents", "Activités enfants"

	// A timed appointment today, concerning two people.
	dentiste, err := svc.Create(ctx, ids["Maman"], events.Input{
		CalendarID: calIDs[famille], Title: "Dentiste Léo",
		StartsAt: at(today, 16, 30), EndsAt: at(today, 17, 15),
		Location: "Cabinet Dr. Roux", LabelID: label(famille, 0),
		Participants: []int64{ids["Maman"], ids["Léo"]},
	})
	if err != nil {
		return fmt.Errorf("seed dentist: %w", err)
	}
	if err := st.ReplaceReminders(ctx, &dentiste.ID, nil, ids["Maman"], []domain.Reminder{
		{EventID: &dentiste.ID, UserID: ids["Maman"], OffsetMinutes: ptr(1440)},
	}); err != nil {
		return err
	}

	if _, err := svc.Create(ctx, ids["Papa"], events.Input{
		CalendarID: calIDs[famille], Title: "Réunion parents-profs",
		StartsAt: at(today.AddDays(1), 18, 0), EndsAt: at(today.AddDays(1), 19, 0),
		Location: "École Jean Moulin", LabelID: label(famille, 1),
		Participants: []int64{ids["Maman"], ids["Papa"]},
	}); err != nil {
		return err
	}
	...
	if _, err := svc.Create(ctx, ids["Maman"], events.Input{
		CalendarID: calIDs[famille], Title: "Vacances à la mer", AllDay: true,
		StartDate: today.AddDays(10), EndDate: today.AddDays(16),
		Location: "Saint-Malo", LabelID: label(famille, 2),
		Participants: []int64{ids["Maman"], ids["Papa"], ids["Léo"], ids["Mamie"]},
	}); err != nil {
		return err
	}
	...
	piscine, err := svc.Create(ctx, ids["Papa"], events.Input{
		CalendarID: calIDs[enfants], Title: "Piscine",
		StartsAt: at(nextTuesday, 17, 30), EndsAt: at(nextTuesday, 18, 30),
		Location: "Piscine municipale", LabelID: label(enfants, 3),
		Participants: []int64{ids["Léo"], ids["Papa"]},
		Recurrence:   &domain.Recurrence{Freq: domain.FreqWeekly, Interval: 1, ByWeekday: []time.Weekday{time.Tuesday}},
	})
	if err != nil {
		return fmt.Errorf("seed piscine: %w", err)
	}
	if piscine.RecurrenceID != nil {
		if err := st.ReplaceReminders(ctx, nil, piscine.RecurrenceID, ids["Papa"], []domain.Reminder{
			{RecurrenceID: piscine.RecurrenceID, UserID: ids["Papa"], OffsetMinutes: ptr(30)},
		}); err != nil {
			return err
		}
	}
	movedDate := nextTuesday.AddDays(7)
	if _, err := svc.Update(ctx, ids["Papa"], piscine.ID, domain.ScopeThis, movedDate, events.Input{
		Title:    "Piscine (horaire exceptionnel)",
	...
	if err := svc.Delete(ctx, ids["Papa"], piscine.ID, domain.ScopeThis, nextTuesday.AddDays(21)); err != nil {
	...
	if _, err := svc.Create(ctx, ids["Maman"], events.Input{
		CalendarID: calIDs[famille], Title: "Anniversaire de Léo", AllDay: true,
		StartDate: birthday, EndDate: birthday, LabelID: label(famille, 4),
		Participants: []int64{ids["Léo"]},
	...
	if _, err := svc.Create(ctx, ids["Maman"], events.Input{
		CalendarID: calIDs[parents], Title: "Relevé des comptes", AllDay: true,
	...
	if _, err := svc.Create(ctx, ids["Papa"], events.Input{
		CalendarID: calIDs[parents], Title: "Cinéma",
		StartsAt: at(saturday, 20, 30), EndsAt: at(saturday, 22, 45),
		LabelID: label(parents, 6), Participants: []int64{ids["Maman"], ids["Papa"]},
	}); err != nil {
		return err
	}
	...
	if _, err := svc.Create(ctx, ids["Maman"], events.Input{
		CalendarID: calIDs[enfants], Title: "Cours de guitare",
		StartsAt: at(wednesday, 14, 0), EndsAt: at(wednesday, 15, 0),
		LabelID: label(enfants, 7), Participants: []int64{ids["Léo"]},
  ...
  Sign in with any of these, password %q:
    maman@example.org   (Maman, admin)
    papa@example.org    (Papa)
    leo@example.org     (Léo)
    mamie@example.org   (Mamie, English UI)

  Calendars: Famille, Parents, Activités enfants
